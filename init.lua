-- symbolhandin.lua : aquietone
-- lua run symbolhandin
-- Choose a zone, TIME or TACVI and then select the items to exchange for symbols
-- Or use commands /giveplanar or /givediscord
-- It will only hand in unattuned items in your main inventory bags. 
-- 
-- WARNING: If you have any unattuned clickies then make sure you don't turn those in as well!
local mq = require('mq')

local zones = { 'TIME','TEXVU','TACVI' }
...
local selectableItems = {['TIME']={},['TEXVU']={},['TACVI']={}}
...
local function selectAll(zone)
    for _,item in ipairs(selectableItems[zone]) do
        item.Selected = true
    end
end
...
local function draw()
    ...
    if show then
        ...
        ImGui.Text('Texvu Loots: ') ImGui.SameLine() ImGui.TextColored(1,1,0,1,'%s', #selectableItems.TEXVU)
        ImGui.SameLine()
        ...
        local tmpselectedZone = ImGui.Combo('Zone', selectedZone, 'Plane of Time\0Texvu\0Tacvi')
        if tmpselectedZone ~= selectedZone then
            selectedZone = tmpselectedZone
        end
        if ImGui.Button('Select All') then
            selectAll(zones[selectedZone])
        end
        ImGui.SameLine()
...
local function givePlanar()
    if doTurnIns then print('Wait for current turninsto complete first!') return end
    selectAll('TIME')
    doTurnIns = true
end

local function giveDiscord()
    if doTurnIns then print('Wait for current turninsto complete first!') return end
    selectAll('TACVI')
    doTurnIns = true
end
...
mq.bind('/givediscord', giveDiscord)
